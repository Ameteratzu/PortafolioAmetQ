...
function applyTheme(m) {
  htmlEl.classList.toggle('dark', m === 'dark')
}
...
<template>
  <div class="min-h-screen flex flex-col bg-gray-50 dark:bg-darkbg">
    ...
    <div class="fixed top-2 right-4 z-50 flex items-center space-x-3">
          ...
          <li v-for="l in locales" :key="l.code" role="menuitem">
            <button
              ...
              class="w-full text-left px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-800 
                     flex items-center space-x-2 text-sm text-gray-900 dark:text-gray-100"
            >
              <span>{{ l.flag }}</span>
              <span>{{ l.label }}</span>
            </button>
      ...
      <button
        ...
        class="flex items-center justify-center w-10 h-10 border-2 rounded-full
               ...
               text-primary dark:text-gray-200 hover:bg-primary/10 dark:hover:bg-accent/10 transition"
        :aria-label="theme==='light'?'Activar modo oscuro':'Activar modo claro'"
      >
        <svg v-if="theme==='light'" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
          <path class="text-primary" d="M10 2a.75.75 0 01.75.75v1.5a.75.75 0 01-1.5 0v-1.5A.75.75 0 0110 2zM10 14a4 4 0 100-8 4 4 0 000 8zm8-4a.75.75 0 01-.75.75h-1.5a.75.75 0 010-1.5h1.5A.75.75 0 0118 10zM4 10a.75.75 0 01-.75.75H1.75a.75.75 0 010-1.5H3.25A.75.75 0 014 10zM15.364 4.636a.75.75 0 011.061 0l1.06 1.06a.75.75 0 11-1.06 1.061l-1.061-1.06a.75.75 0 010-1.061zM5.576 14.424a.75.75 0 011.061 0l1.06 1.06a.75.75 0 01-1.06 1.061l-1.061-1.06a.75.75 0 010-1.061zM15.364 15.364a.75.75 0 010 1.06l-1.061 1.061a.75.75 0 11-1.06-1.06l1.06-1.061a.75.75 0 011.061 0zM5.576 5.576a.75.75 0 010 1.061L4.515 7.698a.75.75 0 11-1.061-1.06l1.06-1.061a.75.75 0 011.062 0z" />
        </svg>
        <svg v-else xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
          <path class="text-accent" d="M17.293 13.293a8 8 0 11-10.586-10.586 7 7 0 0010.586 10.586z" />
        </svg>
      </button>
    </div>
    ...
    <a
      ...
      class="fixed left-4 bottom-4 border-2 border-primary dark:border-accent
             bg-white text-primary dark:text-accent px-4 py-2 rounded-full
             hover:bg-primary/10 dark:hover:bg-gray-700 transition"
    >
      {{ $t('downloadCV') }}
    </a>
  </div>
